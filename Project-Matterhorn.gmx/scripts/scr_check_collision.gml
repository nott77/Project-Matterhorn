///scr_check_collision(x_offset, y_offset);
/*
...
var solid_collision = (place_meeting(xx, yy, par_solid));
...
else
{
    ...
    {
        var semi_collision = (place_meeting(xx, yy, o_semi_solid));
        if (semi_collision)
        {
            ///Only collide with the fallthrough platform
            ///if the area above us is not colliding with the platform
            var above_collision = (place_meeting(xx, yy-1, o_semi_solid));
            if (!above_collision)
            {
                return true;
            }
