...
uniform sampler2D tex_water;             // Water Texture
...
#define xSineCycles 6.128
#define ySineCycles 6.128
...
void main() {
    ...
    //float minRes = min (iResolution.x, iResolution.y); // USER TO KEEP IN SCREEN
    ...
    //uv /= minRes; // USE TO KEEP IN SCREEN
    ...
    //gl_FragColor.rgb = vec3(0.0, 0.2, 0.9) + gl_FragColor.rgb * vec3(0.5, 0.6, 0.1);
}
